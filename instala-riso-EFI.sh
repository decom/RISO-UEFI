...
# Arquivo de instalação do sistema RISO UEFI
...
configurar_boot(){
    ...
    rm -f -v /etc/grub.d/20_memtest86+

    if [ -e /etc/grub.d/10_linux ]; then
            
    mv -v /etc/grub.d/10_linux /etc/grub.d/50_linux
...
instalar_riso() {
    ...
    cp -v ${DIRPATH}/riso /usr/riso/riso
    ...
    cp -v ${DIRPATH}/risos /usr/riso/risos
    ...
    cp -v ${DIRPATH}/rtorrent.rc /root/.rtorrent.rc

    cp -v ${DIRPATH}/riso-EFI.cfg /usr/riso/riso-EFI.cfg

    cp -v ${DIRPATH}/riso-EFI.version /usr/riso/riso-EFI.version

    cp -v ${DIRPATH}/grub.png /grub.png
